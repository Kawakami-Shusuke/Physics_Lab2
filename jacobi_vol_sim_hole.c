    ...
    do {
        ...
        for (int i = 0; i < N; i++){
            for (int j = 0; j < N; j++){
                ...
                //phi_old[i][j] = phi_new[i][j]; //1ステップ前を記録しておく
                double sum = 0;

                // --- 隣接４点を確認 ---
                // 右（i+1）
                //導体紙外縁および空孔なら勾配0
                sum += (i + 1 < N && !is_hole[i + 1][j]) ? phi_old[i + 1][j] : phi_old[i][j];
                // 左 (i-1)
                sum += (i - 1 >= 0 && !is_hole[i - 1][j]) ? phi_old[i - 1][j] : phi_old[i][j];
                // 下 (j+1)
                sum += (j + 1 < N && !is_hole[i][j + 1]) ? phi_old[i][j + 1] : phi_old[i][j];
                // 上 (j-1)
                sum += (j - 1 >= 0 && !is_hole[i][j - 1]) ? phi_old[i][j - 1] : phi_old[i][j];
                ...
                // 相対誤差が最大のものを採用
                if (fabs(phi_new[i][j]) > 1e-10){
                    double diff = fabs(phi_new[i][j] - phi_old[i][j]) / fabs(phi_old[i][j]);
